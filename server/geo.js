...
exports.visual = function( req, res )
{
	...
	postgeo.query( "SELECT imageid AS id, firstdispl || ' - ' || lastdispla AS date, creator, imageviewd AS description, ST_AsGeoJSON( ST_Collect( ST_SetSRID( ST_MakePoint( longitude, latitude ), 4326 ), geom ) ) AS geometry FROM viewsheds WHERE firstdispl <= " + year + " AND lastdispla >= " + year, "geojson", function( data )
	{
		res.send( data );
	});
}
